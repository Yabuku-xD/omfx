...
const Flow = cmd_ctx.Flow;
const PanelKind = cmd_ctx.PanelKind;
const max_extra = cmd_ctx.max_extra;
const Ctx = cmd_ctx.Ctx;
const State = cmd_ctx.State;
const OnOff = cmd_ctx.OnOff;
const BgAction = cmd_ctx.BgAction;
const copy_note = cmd_ctx.copy_note;
const applySurface = cmd_ctx.applySurface;
...
pub fn combinedSpecs(ctx: *Ctx) ![]slash.Spec {
...
pub fn persist(ctx: *Ctx, user: []const u8, assistant: []const u8, trace: agent.Trace, outcome: []const u8) void {
...
pub fn run(ctx: *Ctx, cmd: slash.Name, rest: []const u8) !Flow {
    switch (cmd) {
        .help => {
...
fn formatReload(allocator: std.mem.Allocator, provider: []const u8, model: []const u8, port: u16, skills_n: usize) ![]u8 {
...
fn doPeers(ctx: *Ctx, rest: []const u8) !void {
    ...
    const ep = peerEndpointOf(ctx, rest) orelse {
        try emit(ctx, run_mod.missing_key_text);
        return;
    };
    const json = readAuth(ctx.arena, ctx.io, ctx.home);
...
pub fn doResume(ctx: *Ctx, id_raw: []const u8) !void {
...
pub fn startLoginPick(ctx: *Ctx, rest: []const u8) !void {
    if (rest.len > 0) {
...
pub fn fillWebPick(ctx: *Ctx) void {
    const json = auth.readJson(ctx.gpa, ctx.io, ctx.home);
    defer if (json.len > 0) ctx.gpa.free(json);
...
pub fn startWebPick(ctx: *Ctx, rest: []const u8) !void {
    if (rest.len > 0) {
...
pub fn copyNoteShown(text: []const u8) bool {
...
pub fn copyNote(text: []const u8) CopyNote {
...
fn copyClipboard(io: Io, text: []const u8) bool {
...
pub const PlanArg = union(enum) {
    enter,
    set: agent.Plan,
    go,
    prompt: []const u8,

    pub fn parse(rest: []const u8) PlanArg {
...
pub const RewindArg = union(enum) {
    list,
    steps: usize,
    ...
    pub fn parse(rest: []const u8) ?RewindArg {
