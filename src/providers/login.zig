...
pub fn formatMenu(allocator: std.mem.Allocator, auth_json: []const u8) ![]u8 {
    ...
    for (catalog.all, 0..) |spec, i| {
        ...
        // Tick leads so a clipped menu title still shows configuration.
        const mark: []const u8 = if (on) "✓ " else "";
        ...
        const line = std.fmt.bufPrint(&line_buf, "  {d: >2}  {s}{s: <22} {s}\n", .{ i + 1, mark, spec.id, kind }) catch continue;
