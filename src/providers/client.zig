...
const tool_defs = [_]ToolDef{
    ...
    .{ .name = "web_fetch", .description = "Fetch a public HTTP URL. Returns a short header plus raw body (HTML is truncated; use web_scrape for readable page text)", .parameters = "{\"type\":\"object\",\"properties\":{\"url\":{\"type\":\"string\"}},\"required\":[\"url\"]}" },
    .{ .name = "web_scrape", .description = "Fetch a public HTTP URL and return clean main-page text (title, URL, article/main body; nav and scripts removed)", .parameters = "{\"type\":\"object\",\"properties\":{\"url\":{\"type\":\"string\"}},\"required\":[\"url\"]}" },
    .{ .name = "web_search", .description = "Search the web via omfx backends. Returns numbered title/URL/snippet hits (prefer over any built-in model web search)", .parameters = "{\"type\":\"object\",\"properties\":{\"query\":{\"type\":\"string\"}},\"required\":[\"query\"]}" },
    ...
    .{ .name = "peer", .description = "Spawn a full-capability teammate. Same tools as you. Isolated thread. Communicate via board (FACT/FAIL/PATH). Do not nest peers. Plain words like 'sonnet 5 from anthropic' pin the model; otherwise it auto-routes. Reasoning stays auto unless the goal names a level for that model. The user can also type /peers <goal>.", .parameters = "{\"type\":\"object\",\"properties\":{\"goal\":{\"type\":\"string\"}},\"required\":[\"goal\"]}" },
