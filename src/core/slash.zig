...
pub const builtin = [_]Spec{
    ...
    .{ .name = "/handoff", .help = "new session with a brief; drop the old thread" },
    ...
    .{ .name = "/allowlist", .help = "inspect or append a persistent permission rule" },
    ...
    .{ .name = "/plan", .help = "read-only plan mode; /plan go implements", .args = "[go]" },
